    ...
    const handleRegisterSchedule = async () => {
        if (text && scheduleDateStart && schedule._id) {
            try {
                ...
                const { notification, conversation, schedule_detail } =
                    resp.data;
                if (notification) {
                ...
                if (schedule_detail) {
                    ...
                    dispatch(
                        scheduleDetailSlice.actions.pushScheduleDetail(
                            schedule_detail
                        )
                    );
                    navigation.goBack();
    ...
    return (
        <ScrollView style={styles.container}>
            <List.Section>
                ...
                <List.Item
                    style={styles.list}
                    title={`Bắt đầu: ${moment(scheduleDateStart).format(
                        "llll"
                    )}`}
                ...
                <List.Item
                    style={styles.list}
                    title={`Kết thúc: ${moment(scheduleDateEnd).format(
                        "llll"
                    )}`}
